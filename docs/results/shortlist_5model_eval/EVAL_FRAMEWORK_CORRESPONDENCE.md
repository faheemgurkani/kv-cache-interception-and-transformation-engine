...
**Update 2026-08-19 (94th commit):** TinyDeepSeek support landed — `deepseek_v3` adapter, `project_attention_states()` / MLA split-RoPE path, asymmetric K/V memory, and full eval-branch verification in `tests/test_tinydeepseek_reference.py`. **Falcon-H1 support landed** — `falcon_h1` adapter, hybrid memory accounting, Mamba passthrough; `tests/test_falcon_h1_reference.py`. **5/5 models pass Gate B**; **4/5 pass all three gates** (TinyDeepSeek Gate C still fails: expanded KV, not native latent). See [Current status (2026-08-19)](#current-status-2026-08-19) and the [2026-08-19 implementation audit](../../ENGINE_AND_EVALUATION_FRAMEWORKS_REDESIGN_PLAN.md#implementation-verification-audit-2026-08-19).
