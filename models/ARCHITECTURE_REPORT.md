...
**Engine correspondence — hybrid cache visible; adapter gate still blocks FIDELITY/attention:**
- `load_attention_ops` → **fails**: `NotImplementedError("... model_type='falcon_h1' ...")`.
...
- Gate C → **passes** when all visible components are accounted for; compression policy remains attention-K/V-only (Mamba passthrough).
...
- **FIDELITY/attention would additionally crash even with an adapter added**, unless per-layer RoPE is selected: `eval/fidelity/attention.py` uses `framework.rope.build_rope_context(...).get_rope(layer_idx)` (WP1), but `gemma3_text` is still not registered in `ATTENTION_ADAPTER_REGISTRY`, so Gate B fails before a full FIDELITY run completes.
