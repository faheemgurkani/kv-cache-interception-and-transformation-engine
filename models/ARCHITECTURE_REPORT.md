...
- Also needs: a third `qk_norm_layout` value (`"none"`) in `project_qkv` (WP1 scaffold present), and a `model_type == "falcon_h1"` branch in `load_attention_ops`. Memory accounting via `visible_state_bytes()` now includes Mamba state; compression remains K/V-only.
